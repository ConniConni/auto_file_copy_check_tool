...
- **選択的コピー**: ファイルタイプ選択（ドキュメントのみ/レビュー記録表のみ）と個別ファイル選択が可能
...
5. **動作モード**:
   ...
   - 2: Incoming (外部→内部: 取込)
6. **ファイルタイプ選択**:
   - 1: すべて（ドキュメント、レビュー記録表、例外ファイル）
   - 2: ドキュメントのみ（ドキュメント本体と例外ファイル）
   - 3: レビュー記録表のみ
7. **期間フィルタ**: N日前以降に更新されたファイルのみを対象
   - 0: 今日のみ
   - 1: 昨日以降
   - 7: 7日前以降
8. **個別ファイル選択**: スキャン結果から番号でコピー対象を選択
   - カンマ区切り（例: 1,3,5）
   - 範囲指定（例: 1-5,7,9-11）
   - 'all' ですべて選択
   - Enter キーで処理を中止
...
=== ファイル一覧 ===
  ...
  1. [ドキュメント] 調査検討書_DeveloperSite対応_ディベロッパサイト環境移行対応.xlsx
  2. [レビュー記録表] レビュー記録表(社外)_1回目_調査_DeveloperSite対応.xlsx [OK]
  3. [レビュー記録表] レビュー記録表(社外)_2回目_調査_DeveloperSite対応.xlsx [OK]

合計: 3件

コピーするファイルを選択してください:
- 番号をカンマ区切りで指定 (例: 1,3,5)
- 範囲指定も可能 (例: 1-5,7,9-11)
- 'all' ですべて選択
- Enter キーで処理を中止
...
ログファイル: log/file_copy_tool_20251223_143025.log
```

**例4: レビュー記録表のみを選択的にコピー**
...
期間フィルタ（N日前以降、0=今日のみ）: 7

=== ファイル一覧 ===

  1. [レビュー記録表] レビュー記録表(社外)_1回目_調査_xxx.xlsx [OK]
  2. [レビュー記録表] レビュー記録表(社外)_2回目_調査_xxx.xlsx [NG: AE7が空です]
  3. [レビュー記録表] レビュー記録表(社外)_3回目_調査_xxx.xlsx [OK]
...
- 2025-12-24:
  - 選択的コピー機能追加
    - ファイルタイプ選択機能（すべて/ドキュメントのみ/レビュー記録表のみ）
    - 個別ファイル選択機能（番号指定、範囲指定、カンマ区切り対応）
  - ユーザビリティ向上
    - 番号付きファイル一覧表示
    - レビュー記録表のチェック結果を選択時に表示
  - ログファイル出力先を`log/`ディレクトリに変更（自動作成）
